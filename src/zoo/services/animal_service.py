...
class AnimalService:
    ...
    def __init__(self, animal_repository):
        self.animal_repository: AnimalRepository = animal_repository
    ...
    def add_animal(self, animal):
        ...
        if not animal_name or not isinstance(animal_name, str) or len(animal_name) < 3:
            raise ValueError(
                "Animal name must be a string and have 3 character minimun"
            )
        ...
        if species not in [species.value for species in ListSpecies]:
            raise ValueError(
                "Animal species must be on of : Carnivora, Omnivora or Herbivora"
            )
